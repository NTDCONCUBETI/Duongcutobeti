...
local gui = plr:WaitForChild("PlayerGui"):FindFirstChild("ElgatoUI") or Instance.new("ScreenGui", plr:WaitForChild("PlayerGui"))
gui.Name, gui.IgnoreGuiInset, gui.ZIndexBehavior, gui.ResetOnSpawn = "ElgatoUI", true, Enum.ZIndexBehavior.Global, false

local c1 = {Signals = {}}
function c1.AddSignal(sig, func)
	local con = sig:Connect(func)
	table.insert(c1.Signals, con)
	return con
end

local function n1(class, props, kids)
	local i = Instance.new(class)
	for p, v in pairs(props or {}) do i[p] = v end
	for _, k in ipairs(kids or {}) do k.Parent = i end
	return i
end

local r = game:GetService("RunService")
local isMobile = not r:IsStudio() and table.find({Enum.Platform.IOS, Enum.Platform.Android}, uis:GetPlatform()) ~= nil
local size = isMobile and UDim2.fromOffset(42, 42) or UDim2.fromOffset(55, 55)
gui.ResetOnSpawn = false

local btn = n1("ImageButton", {
	Name = "MinimizeButton",
	Size = size,
	Position = UDim2.fromScale(0.1, 0.23),
	...
	n1("UICorner", {CornerRadius = UDim.new(0, 7)})
})

local d1, dragInput, mPos, sPos

local function startDrag(input)
	d1 = true
	mPos = input.Position
	sPos = btn.Position
	c1.AddSignal(input.Changed, function()
		if input.UserInputState == Enum.UserInputState.End then d1 = false end
	end)
end

local function updateDrag(input)
	if input == dragInput and d1 then
		local delta = input.Position - mPos
		local vs = cam.ViewportSize
		local inset = gs:GetGuiInset()
		local nx = math.clamp(sPos.X.Scale + (delta.X / vs.X), 0, 1)
		local ny = math.clamp(sPos.Y.Scale + (delta.Y / (vs.Y + inset.Y)), 0, 1)
		btn.Position = UDim2.fromScale(nx, ny)
	end
end

c1.AddSignal(btn.InputBegan, function(i)
	if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then
		startDrag(i)
	end
end)

c1.AddSignal(btn.InputChanged, function(i)
	if i.UserInputType == Enum.UserInputType.MouseMovement or i.UserInputType == Enum.UserInputType.Touch then
		dragInput = i
	end
end)

c1.AddSignal(uis.InputChanged, function(i)
	updateDrag(i)
end)

c1.AddSignal(btn.MouseButton1Click, function()
